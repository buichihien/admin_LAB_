...
const BookingRoom = () => {
    ...
    const [mssvOptions, setMssvOptions] = useState([]); // Trạng thái cho các MSSV từ db
    const [selectedMssv, setSelectedMssv] = useState(null); // Trạng thái cho MSSV đã chọn
    ...
    useEffect(() => {
        // Lấy thông tin người dùng khi MSSV thay đổi
        const fetchUserData = async () => {
            if (selectedMssv) {
                const q = query(collection(db, "Users"), where("mssv", "==", selectedMssv.value));
                const querySnapshot = await getDocs(q);
                if (!querySnapshot.empty) {
                    const userData = querySnapshot.docs[0].data();
                    setData({ username: userData.username, mssv: userData.mssv, class: userData.class, phone: userData.phone });
    ...
    useEffect(() => {
        // Lấy tất cả MSSV từ db để hiển thị trong dropdown
        const fetchMssvOptions = async () => {
            ...
            <div className="formbold-form-wrapper">
                ...
                <form onSubmit={handleADD}>
                    <div className="mssv">
                        <Select
                            options={mssvOptions}
                            onChange={(selectedOption) => setSelectedMssv(selectedOption)}
